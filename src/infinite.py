...
def compute_and_save(batched_kernel_fn, trX, trY,
    ...
    if K_tr_tr is None:
        logging.info('Computing training kernel matrix...')
        t0_tr = time.time()
    ...
    if K_val_tr is None:
        logging.info('Computing validation kernel matrix...')
        t0_val = time.time()
    ...
    if K_test_tr is None:
        logging.info('Computing test kernel matrix...')
        t0_test = time.time()
    ...
    logging.info('[NNGP] Validation score is {}'.format(val_score_nngp))
    logging.info('[NNGP] Test score is {}'.format(test_score_nngp))
    logging.info('[NTK] Validation score is {}'.format(val_score_ntk))
    logging.info('[NTK] Test score is {}'.format(test_score_ntk))
    results_obj = {
    ...
    if dump:
        logging.info('Inference finished. Dumping logs...')
